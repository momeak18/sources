...
fn get_page_list(_: String, chapter_id: String) -> Result<Vec<Page>> {
	...
	let list_start = match text.find("let listimg=") {
		Some(index) => index + "let listimg=".len(),
		None => return Err(default_error()),
	};
	let list_end = match text[list_start..].find(";") {
		Some(index) => index + list_start,
		None => return Err(default_error()),
	};
	let list = &text[list_start..list_end];
	let list = json::parse(list).map_err(|_| json_error())?.as_array()?;
...
fn modify_image_request(request: Request) {
	let _ = helper::add_common_headers(request, &helper::referer_url());
}
